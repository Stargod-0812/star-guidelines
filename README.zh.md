...
**非 solid 工程基础** — Star Guidelines 是安全网。

这类使用者最大的风险是看不出 agent 什么时候在 drift：分不清 scope creep 和正常实现，分不清过度抽象和合理封装，分不清伪造完成和真实验证。装上 Star Guidelines 之后，agent 在执行层自带 guardrail，等于借了一套高级工程判断力。从「不知道自己不知道」变成「agent 替我守住底线」。

**Solid 工程基础** — Star Guidelines 是注意力释放器。

这类使用者能一眼看出 agent 的问题，但不应该每次都需要手动 enforce。没有 Star Guidelines，每轮对话都要重复：「不要改别的文件」「跑个测试再说完成」「不要自作主张加抽象」。装上之后省去了 babysitting——agent 预装了你的工程标准，你只需要 review 结果而不是 supervise 过程。

**共同点**：用 agent 越频繁、委托的任务越重，Star Guidelines 的价值越大。它不教你写代码，它教 agent 不要乱来。
...
自由使用：阅读、学习、评估，以及在保留署名和链接的前提下引用片段。

需书面授权：分发、镜像、打包进产品、在此基础上构建竞品规则集、用于模型训练。
